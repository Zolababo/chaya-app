...
export function GuestOrderHistoryCard({
  ...
  return (
    ...
    >
      <Link
        href={href}
        className={`block transition active:opacity-90 ${easyMode ? "min-h-[52px] px-4 py-4" : "px-3 py-3"}`}
        aria-label={`${cardTitle} ${orderNoText}, ${formatConsumerMoney(order.total_price, locale)}, ${linkAria}`}
      >
        {cancelledNotice ? (
          <p
            role="status"
            className={`mb-2 rounded-lg border border-red-200 bg-red-50 px-2.5 py-1.5 font-semibold text-red-800 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-200 ${easyMode ? "text-sm" : "text-xs"}`}
          >
            {cancelledNotice}
          </p>
        ) : null}
        <div className={`flex items-start justify-between gap-2 ${easyMode ? "mb-3" : "mb-2"}`}>
          <div className="min-w-0">
            <p
              className={`font-extrabold text-zinc-900 dark:text-zinc-50 ${easyMode ? "text-xl" : "text-base"}`}
            >
              {orderNoText}
            </p>
          </div>
