      ...
      {filtered.length > 0 ? (
        <ul aria-label={m.barrierFree.menuList} className={menuCardListClass}>
          {filtered.map((raw) => {
            ...
            const priceLabel = formatConsumerMoney(item.price, locale);
            const detailHref = navHref(
              `/t/${encodeURIComponent(slug)}/menu/${encodeURIComponent(item.id)}`,
            );
            const detailAriaLabel = `${name}, ${priceLabel}. ${m.barrierFree.detailAria.replace("{name}", name)}`;
            return (
                ...
                <MenuListRow
                  ...
                  trailing={
                    item.isSoldOut ? (
                      ...
                      <button
                        type="button"
                        aria-label={`${name}, ${priceLabel}. ${m.menu.addToCart}`}
