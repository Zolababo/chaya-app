...
현재 레포에서는 점주 화면이 **별도 앱이 아니라**, 동일한 Next 앱 안의 **`/m/[tenant]/...`** 경로(주문 큐·메뉴 관리 등)로 **일부만** 제공된다. 배포·실험에는 유리하지만, 접근이 **URL 토큰·쿠키 중심**이면 직원 계정·초대·분실·감사 설명 같은 **현장 요구와 거리가 남을 수 있다**.
...
|------|----------------------------------|
| **신뢰·보안** | 누가 어떤 가게 데이터에 접근하는지 **설명 가능**(계정·초대·역할 또는 합의된 대안). URL 토큰만으로는 유출·재발급 시나리오를 말하기 어렵다. |
