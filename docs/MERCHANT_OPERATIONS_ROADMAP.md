...
| 영역 | 비율 | 비고 |
|------|------|------|
| 점주 콘솔 운영 성숙도(로드맵 Phase 1–4 통합) | **~78%** | Phase 1 완료 + 감사 **조회**까지. CSV·`/ops` 교차조회·알림·세분 RBAC는 미완. |
| 소비자 `/t` 주문·메뉴 MVP | **~62%** | 게스트 주문·장바구니·주문 조회 등 핵심은 동작; 결제/계정/직원호출 등은 제품 단계에 따라 확장. |
