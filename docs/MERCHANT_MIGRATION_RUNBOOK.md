...
- Logout: POST `/m/logout` clears the session and redirects to **`/m/login`** (optional sanitized `next`); GET returns **405**.
...
- 역할: `owner`, `staff`, `menu_editor`, `viewer`, `finance`(정산 조회용; 앱 권한은 viewer와 동일). `finance` 는 마이그레이션 `20260514100000_merchant_tenant_members_finance_role.sql` 적용 후 insert 가능.
...
-- role 은 owner | staff | menu_editor | viewer | finance (마이그레이션 phase4 + finance 적용 후)
```
