...
- 경로: `/t/{tenant}/barrier-free` — **하단 탭에는 두지 않음**. 헤더 **「큰글씨·목록」** 으로 진입 (QR 모바일 UX 우선).
- **큰글씨·목록(Big UI):** `sessionStorage` 로 모드 유지 — 목록·장바구니·주문·상세까지 글자·버튼·터치(52px) 확대. `html[data-consumer-easy-mode]` 전역 스케일.
...
**지금 소리로 테스트하기 어렵다면** 이 절은 **후순위**로 두어도 됩니다. (앱 구현·배포·DB 검증과 별개입니다.) 그동안 **데스크톱 크롬에서 `Tab` / `Shift+Tab`** 만으로 스킵 링크·주요 버튼까지 **포커스가 순서대로 보이는지** 빠르게 훑어보는 정도만 권장합니다. 이것은 스크린리더 검증을 대체하지 않습니다.
...
4. **장바구니**: 줄별 삭제·수량·주문 보내기 라벨, 오류·로딩 메시지가 읽히는지. 「주문 보내기」에 연결된 **같은 폰·같은 브라우저** 안내(`aria-describedby`)가 버튼과 함께 자연스럽게 읽히는지.
5. **주문 목록·상세**: 목록 링크 안내, 각 행 **링크 복사** 후 `aria-live` 안내가 읽히는지, 상태 갱신(폴링) 시 과도한 반복이 없는지(필요 시 동작 줄이기 설정으로 폴링 꺼짐 확인).
