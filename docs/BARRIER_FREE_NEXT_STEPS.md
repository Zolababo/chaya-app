...
1. **실기기 접근성**: 위 체크리스트를 TalkBack·VoiceOver로 끝까지 돌리고, 발견한 이슈만 이슈 트래커에 기록.
2. **제품 합의** (`docs/QR_AND_GUEST_ORDERS.md` §3): QR 경로 기본안, 테이블 번호 필수 여부, 과거 주문 찾기(세션만 vs 주문번호+전화 등).
3. **콜드 링크·SMS**: 세션 없이 주문 URL만 열었을 때의 UX·보안(서명 토큰·만료·OTP 등)을 정한 뒤 RPC·앱을 같이 설계.
4. **회원 연동**: 비회원 `guest_session_id` → 로그인 후 주문 묶기(claim) 정책·구현.
5. **관측**: Vercel/Supabase 로그·알림, 주문 실패율, `/health` 모니터링을 운영 루틴에 넣기.
