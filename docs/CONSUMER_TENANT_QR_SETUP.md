...
1. `/m/login` 로그인
2. **`/m/{tenant_slug}/tables`** — 번호 입력 → **CHAYA가 QR 생성** → 카드에서 **인쇄** / **QR 저장** · 상단 **전체 QR 인쇄** · **QR ZIP 받기**
3. 손님은 등록된 테이블만 주문 가능(QR 잠금·목록 선택·서버 검증). 테이블이 없으면 장바구니에서 번호 직접 입력(레거시).

대시보드 **「손님 QR·메뉴 주소」** 카드: 공통 메뉴 URL·테이블 예시 복사(수동 QR용).
