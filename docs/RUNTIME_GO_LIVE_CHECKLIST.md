...
### 3-3. (권장) 점주 역할 확장 — Phase 4

`/ops/merchants` 에서 **menu_editor·viewer·finance** 등을 쓰려면, `merchant_tenant_members.role` 제약이 DB와 맞아야 합니다. 아래를 **순서대로** 한 번씩 실행합니다(이미 돌렸으면 스킵).

1. `supabase/migrations/20260513190000_merchant_tenant_members_phase4_roles.sql`  
2. `supabase/migrations/20260514100000_merchant_tenant_members_finance_role.sql`

배포된 앱의 **`GET /health`** JSON 안 `supabase.merchantMemberRoles` 에도 지원 역할·파일명 힌트가 있습니다.
...
## 선택: 점주 브라우저 웹 푸시 (새 주문 알림)

**의미:** 없어도 주문·메일·대시보드 피드는 동작합니다. **브라우저 푸시**까지 쓰려면 추가로 합니다.

1. Supabase에 **`20260512220000_merchant_push_subscriptions.sql`** 적용(알림 구독 표 + RLS).  
2. 로컬에서 `npx web-push generate-vapid-keys` 로 키를 만들고, Vercel에 **`NEXT_PUBLIC_VAPID_PUBLIC_KEY`**, **`VAPID_PRIVATE_KEY`**, **`VAPID_SUBJECT`**(예: `mailto:you@domain`) 를 넣은 뒤 **재배포**.  
3. 자세한 순서·주의사항은 **`docs/MERCHANT_MIGRATION_RUNBOOK.md` → 「5-c) 브라우저 웹 푸시」** 를 보세요.
...
- **점주 초대에서 역할 저장이 실패한다**  
  → `merchant_tenant_members` 의 `role` CHECK가 옛날이면 `20260513190000_*`·`20260514100000_*` 마이그레이션을 적용했는지 확인. `GET /health` 의 `supabase.merchantMemberRoles` 참고.
- **점주 대시보드에서 브라우저 알림이 안 켜진다**  
  → VAPID 세 환경 변수를 넣고 재배포했는지, Supabase에 `merchant_push_subscriptions` 마이그레이션을 적용했는지 확인합니다. (`MERCHANT_MIGRATION_RUNBOOK` 5-c)
...
| (권장) 점주 역할 Phase 4 | `20260513190000_*` → `20260514100000_*` (`/health` 의 `merchantMemberRoles` 참고) |
| 운영자 1명 | Auth에서 유저 만들기 → `platform_operators`에 UUID insert |
| 점주 만들기 | `/ops/merchants` 폼 또는 SQL |
| (선택) 점주 웹 푸시 | DB 마이그레이션 `20260512220000_*` + VAPID 세 변수 + 재배포 (`MERCHANT_MIGRATION_RUNBOOK` 5-c) |
...
프로덕션 프로젝트는 Supabase 대시보드에서 **일일 백업** 한도와 **Point-in-time recovery(PITR)** 옵션(유료 플랜)을 확인하고, 장애 시 **복구 절차**를 팀이 숙지해 두는 것을 권장합니다. 접근 자격·키는 저장소가 아니라 비밀 저장소·대시보드에만 둡니다.
